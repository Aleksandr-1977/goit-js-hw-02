...
function getElementWidth(content, padding, border) {
  const _content = Number.parseFloat(content);
  const _padding = Number.parseFloat(padding);
  const _border = Number.parseFloat(border);

  return _content + _padding * 2 + _border * 2;
}
